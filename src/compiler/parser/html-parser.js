  ...
  while (html) {
    ...
    if (!lastTag || !isPlainTextElement(lastTag)) {
      ...
      if (textEnd === 0) {
        ...
        // 获取到开始标签和开始标签中的属性并添加到match.attr中，比如 <div id="app">，startTagMatch = { tagName: 'div', attrs: [[xx], ...], start: index }
 ...
 */
  ...
  */
  function handleStartTag (match) {
    ...
    if (expectHTML) {
      // 如果p元素嵌套了不能存放在p中的元素就手动调用parseEndTag 例如 <p>123<div>234</div></p> 手动给p进行结束变成<p>123</p><div>234</div></p>
      // 然后在parseEndTag调用的时候进行相对应的处理
  ...
  */
  ...
  parseEndTag 的核心逻辑很简单，在介绍之前我们回顾一下在执行 handleStartTag 的时候，对于非一元标签（有 endTag）我们都把它构造成一个对象压入到 stack 中，
