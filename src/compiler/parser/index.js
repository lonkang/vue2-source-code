...
let maybeComponent
// 每一个 AST 元素就是一个普通的 JavaScript 对象，其中，type 表示 AST 元素类型，tag 表示标签名，attrsList 表示属性列表，attrsMap 表示属性映射表，parent 表示父的 AST 元素，children 表示子 AST 元素集合。
 ...
 */
    start (tag, attrs, unary, start, end) {
      // 当解析到开始标签的时候，最后会执行 start 回调函数，函数主要就做 3 件事情，创建 AST 元素，处理 AST 元素，AST 树管理。
      ...
      */
      ...
      // 接着判断 element 是否包含各种指令通过 processXXX 做相应的处理处理的结果就是扩展 AST 元素的属性
  ...
  for (i = 0, l = list.length; i < l; i++) {
    ...
    if (dirRE.test(name)) {
      ...
      if (bindRE.test(name)) { // v-bind, <div :id="test"></div>
        ...
        // 属性名，即事件名
        name = name.replace(onRE, '') // name:click
        // 是否为动态属性
        isDynamic = dynamicArgRE.test(name)
        if (isDynamic) {
          // 动态属性，则获取 [] 中的属性名
